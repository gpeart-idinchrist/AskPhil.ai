...
Returns:
  Updated settings object.`,
    ...
    async (params: UpdateSettingsInput) => {
      ...
      if (params.system_prompt !== undefined) s.systemPrompt = params.system_prompt;
      if (params.model_name !== undefined) s.modelName = params.model_name;
      if (params.temperature !== undefined) s.temperature = params.temperature;
      if (params.max_tokens !== undefined) s.maxTokens = params.max_tokens;
      if (params.welcome_message !== undefined) s.welcomeMessage = params.welcome_message;
      if (params.brand_name !== undefined) s.brandName = params.brand_name;
      if (params.enable_knowledge !== undefined) s.enableKnowledge = params.enable_knowledge;
      if (params.enable_chat_history !== undefined) s.enableChatHistory = params.enable_chat_history;

      saveStore(store);

      return {
        content: [{ type: "text" as const, text: "Settings updated successfully." }],
        structuredContent: s,
      };
    }
  );
