...
import { UploadDocumentSchema, ListDocumentsSchema, GetDocumentSchema, DeleteDocumentSchema, SearchKnowledgeSchema, UpdateDocumentSchema, } from "../schemas/knowledge.js";
...
export function registerKnowledgeTools(server) {
    // ─── Upload Document ────────────────────────────────────
    server.registerTool("askphilai_upload_document", {
...
Error Handling:
  - Returns error if content exceeds 5 MB.`,
        ...
        if (sizeBytes > MAX_DOCUMENT_SIZE) {
            return {
                isError: true,
                content: [{ type: "text", text: `Error: Document exceeds maximum size of ${MAX_DOCUMENT_SIZE / 1024 / 1024} MB. Current size: ${(sizeBytes / 1024 / 1024).toFixed(2)} MB.` }],
            };
        }
        ...
        return {
            content: [{ type: "text", text: `Document uploaded.\n\n${formatDocSummary(doc)}` }],
            structuredContent: output,
        };
    });
    // ─── List Documents ─────────────────────────────────────
    server.registerTool("askphilai_list_documents", {
...
Returns:
  Paginated list of document summaries.`,
        ...
        const store = loadStore();
        let docs = Object.values(store.documents).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
        if (params.tag) {
        ...
        if (params.search) {
            ...
            docs = docs.filter((d) => d.filename.toLowerCase().includes(q) || d.content.toLowerCase().includes(q));
        }
    ...
    // ─── Get Document ───────────────────────────────────────
    server.registerTool("askphilai_get_document", {
        title: "Get Document",
...
Returns:
  Complete document object with content.`,
        ...
        if (!doc) {
            return {
                isError: true,
                content: [{ type: "text", text: `Error: Document '${params.document_id}' not found. Use askphilai_list_documents to see available documents.` }],
            };
        }
        const text = truncate([`# ${doc.filename}`, `**ID:** ${doc.id}`, `**Type:** ${doc.mimeType}`, `**Size:** ${(doc.sizeBytes / 1024).toFixed(1)} KB`, `**Tags:** ${doc.tags.join(", ") || "none"}`, `**Created:** ${doc.createdAt}`, `**Updated:** ${doc.updatedAt}`, "", "---", "", doc.content].join("\n"), CHARACTER_LIMIT);
        return {
            content: [{ type: "text", text }],
    ...
    // ─── Update Document ────────────────────────────────────
    server.registerTool("askphilai_update_document", {
...
Returns:
  Updated document summary.`,
        ...
        if (!doc) {
            return {
                isError: true,
                content: [{ type: "text", text: `Error: Document '${params.document_id}' not found.` }],
            };
        }
        if (params.filename)
        ...
        if (params.content) {
            ...
            if (sizeBytes > MAX_DOCUMENT_SIZE) {
                return {
                    isError: true,
                    content: [{ type: "text", text: `Error: Updated content exceeds maximum size of ${MAX_DOCUMENT_SIZE / 1024 / 1024} MB.` }],
                };
            }
        ...
        saveStore(store);
        return {
            content: [{ type: "text", text: `Document updated.\n\n${formatDocSummary(doc)}` }],
            structuredContent: { id: doc.id, filename: doc.filename, updatedAt: doc.updatedAt },
        };
    });
    // ─── Delete Document ────────────────────────────────────
    server.registerTool("askphilai_delete_document", {
...
Returns:
  Confirmation with deleted document's filename.`,
        ...
        saveStore(store);
        return {
            content: [{ type: "text", text: `Document "${filename}" (${params.document_id}) deleted.` }],
            structuredContent: { deleted: true, id: params.document_id, filename },
        };
    });
    // ─── Search Knowledge ───────────────────────────────────
    server.registerTool("askphilai_search_knowledge", {
...
Examples:
  ...
  - "Search for deployment steps" → query="deployment"`,
        ...
        for (const doc of docs) {
            ...
            if (snippets.length >= params.limit)
                break;
        }
        ...
        const lines = snippets.map((s, i) => `### ${i + 1}. ${s.filename} (pos ${s.position})\n\`\`\`\n${s.snippet}\n\`\`\``);
        const text = truncate([`Found ${snippets.length} match(es) for "${params.query}"`, "", ...lines].join("\n"), CHARACTER_LIMIT);
        return {
            content: [{ type: "text", text }],
