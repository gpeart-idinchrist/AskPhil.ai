...
import { CreateChatSchema, SendMessageSchema, ListChatsSchema, GetChatSchema, DeleteChatSchema, UpdateChatSchema, } from "../schemas/chat.js";
...
function formatChat(chat, includeMessages) {
    const lines = [
        `# ${chat.title}`,
        `**ID:** ${chat.id}`,
...
export function registerChatTools(server) {
    // ─── Create Chat ────────────────────────────────────────
    server.registerTool("askphilai_create_chat", {
        title: "Create Chat",
...
Examples:
  ...
  - "Create a customer support chat" → title="Support #1234", metadata={"customer":"1234"}`,
        ...
        return {
            content: [{ type: "text", text: `Chat created successfully.\n\n${formatChat(chat, false)}` }],
            structuredContent: output,
        };
    });
    // ─── Send Message ───────────────────────────────────────
    server.registerTool("askphilai_send_message", {
        title: "Send Message",
...
Error Handling:
  - Returns error if chat_id does not exist.`,
        ...
        if (!chat) {
            return {
                isError: true,
                content: [{ type: "text", text: `Error: Chat '${params.chat_id}' not found. Use askphilai_list_chats to see available chats.` }],
            };
        }
        const message = {
            id: uuidv4(),
        ...
        return {
            content: [{ type: "text", text: `Message added to "${chat.title}".\n\n**[${message.role}]** ${truncate(message.content, 200)}` }],
            structuredContent: output,
        };
    });
    // ─── List Chats ─────────────────────────────────────────
    server.registerTool("askphilai_list_chats", {
        title: "List Chats",
...
Returns:
  Paginated list of chat summaries (id, title, message count, timestamps).`,
        ...
        const store = loadStore();
        let chats = Object.values(store.chats).sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime());
        if (params.search) {
            const q = params.search.toLowerCase();
            chats = chats.filter((c) => c.title.toLowerCase().includes(q));
        }
        const page = paginate(chats, params.offset, params.limit);
        const lines = page.items.map((c) => `- **${c.title}** (${c.id}) – ${c.messages.length} messages, updated ${c.updatedAt}`);
    ...
    // ─── Get Chat ───────────────────────────────────────────
    server.registerTool("askphilai_get_chat", {
        title: "Get Chat",
...
Error Handling:
  - Returns error if chat_id does not exist.`,
    ...
    // ─── Update Chat ────────────────────────────────────────
    server.registerTool("askphilai_update_chat", {
        title: "Update Chat",
...
Returns:
  Updated chat summary.`,
        ...
        if (!chat) {
            return {
                isError: true,
                content: [{ type: "text", text: `Error: Chat '${params.chat_id}' not found.` }],
            };
        }
        if (params.title)
            chat.title = params.title;
        if (params.metadata)
            chat.metadata = { ...chat.metadata, ...params.metadata };
        chat.updatedAt = new Date().toISOString();
        saveStore(store);
        return {
            content: [{ type: "text", text: `Chat updated.\n\n${formatChat(chat, false)}` }],
            structuredContent: { id: chat.id, title: chat.title, updatedAt: chat.updatedAt },
        };
    });
    // ─── Delete Chat ────────────────────────────────────────
    server.registerTool("askphilai_delete_chat", {
        title: "Delete Chat",
...
Error Handling:
  - Returns error if chat_id does not exist.`,
        ...
        saveStore(store);
        return {
            content: [{ type: "text", text: `Chat "${title}" (${params.chat_id}) deleted.` }],
